# openpi × 天工机器人（TienKung）

本仓库基于 [Physical Intelligence 的 openpi](https://github.com/Physical-Intelligence/openpi) 二次开发，**主要工作是以 π₀.₅（pi0.5）为基座模型，在天工双臂机器人（软夹爪、3 相机）上进行微调训练与部署**。当前任务：使用粘毛滚筒清理桌面碎屑（"use a lint roller to pick up crumbs on the desk"）。

## 项目概览

- **基座模型**：`pi05_base`（本地路径 `checkpoints/pi05_base`），JAX 全参微调
- **数据集**：LeRobot v2.1 格式，401 条 episode，位于 `datasets/tienkung_lint_roller_lerobot`（训练时需设置 `HF_LEROBOT_HOME` 指向 `datasets` 目录）
- **动作空间**：关节空间 16 维 = 左臂 7 关节 + 右臂 7 关节 + 左右夹爪各 1 维；`action_horizon=32`
- **训练配置**（定义于 `src/openpi/training/config.py`）：
  - `pi05_tienkung_lint_roller_delta`：主线，关节 delta 动作 + 夹爪绝对值
  - `pi05_tienkung_lint_roller_abs`：对照，关节绝对动作

## 相对上游的主要改动

| 位置 | 内容 |
| --- | --- |
| `src/openpi/policies/tienkung_policy.py` | 天工输入/输出变换（`TienkungInputs`/`TienkungOutputs`），3 相机映射、图像格式转换、动作维度截断 |
| `src/openpi/training/config.py` | `TienkungRepack`（4 个 puppet 字段拼接为 16 维）、`LeRobotTienkungDataConfig`、两套 `TrainConfig` |
| `src/openpi/training/data_loader.py` | `drop_n_last_frames` 支持（`EpisodeAwareSampler`），避免动作块跨越 episode 边界 |
| `scripts/patch_norm_stats.py` | **新增**：对近恒定维度（如闲置左臂）的 quantile 范围加下限，消除归一化放大导致的 loss 尖峰 |
| `scripts/train.py` | 日志增强：自动保存 `logs/train_*.log` 与 `logs/metrics_*.jsonl` 到 checkpoint 目录；W&B 新增 `loss_median`/`loss_max`/`lr` 指标 |
...
# 1. 计算归一化统计
python scripts/compute_norm_stats.py --config-name pi05_tienkung_lint_roller_delta

# 2. 修补近恒定维度的 quantile 范围（重要，防止 loss 尖峰）
python scripts/patch_norm_stats.py --config-name pi05_tienkung_lint_roller_delta

# 3. 启动训练（单机 8 卡 FSDP；当前 JAX 训练脚本不支持多机）
XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 python scripts/train.py pi05_tienkung_lint_roller_delta \
    --exp-name my_experiment --fsdp-devices 8

# 4. 部署：启动策略服务（机器人侧通过 websocket 查询）
python scripts/serve_policy.py policy:checkpoint \
    --policy.config=pi05_tienkung_lint_roller_delta \
    --policy.dir=checkpoints/pi05_tienkung_lint_roller_delta/my_experiment/29999
```

## 项目文档

- [`docs/微调方案.md`](docs/微调方案.md)：完整微调方案（数据、变换、超参、风险与修复记录）
- [`agent.md`](agent.md)：项目上下文与决策备忘
- [`docs/环境复现记录.md`](docs/环境复现记录.md)：环境搭建记录

---

以下为 openpi 官方原始 README。
