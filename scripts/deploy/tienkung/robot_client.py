#!/usr/bin/env python3
"""TienKung dual-arm remote inference client (blocking chunk execution).

Adapted from the user's previous-project client (参考/client.py) for the
pi05_tienkung_lint_roller_delta checkpoints. Runs on the robot host with the
old project's Python environment (xRocs / httpx / cv2), NOT this repo's venv.

LAYOUT CHANGE vs the reference client -- the whole chain now uses the TRAINING
layout (see docs/推理部署方案.md):
...
Every index in this file follows this layout. Do NOT copy indices from the
reference client (it used [L7, Lg, R7, Rg]).
"""
...
DEFAULT_TASK = "Use an electrostatic roller to pick up debris on the desktop"  # 必须与训练 prompt 逐字一致
...
# lint roller 数据集 401 条 episode 首帧位姿均值(训练布局 [L7, R7, Lg, Rg])
...
class TienkungInferenceClient:
    def __init__(self, config_path: str, task_name: str, infer_url: str = INFER_URL, *, dry_run: bool = False):
        ...
        self.http_client = httpx.Client(
            ...
            headers={
                "User-Agent": f"TienkungClient/{urllib.parse.quote(task_name)}",
    ...
    def _pick_camera(self, images: dict[str, np.ndarray], short_key: str) -> np.ndarray | None:
        for key, frame in images.items():
            if key.lower() == short_key:
                return frame
        for key, frame in images.items():
            if short_key in key.lower():
                return frame
        return None
    ...
    def prepare(self):
        print("移动到 lint roller 数据集首帧均值位姿。")
        ...
        logger.success("Tienkung dual-arm prepare done.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TienKung dual-arm remote inference client")
