"""HTTP inference server for the TienKung dual-arm robot (pi05_tienkung_* configs).

Adapted from the user's previous-project server (参考/server.py). The whole chain uses
the TRAINING layout [left arm 7, right arm 7, left gripper 1, right gripper 1] --
the state from the client is passed through to `observation/state` unchanged, and the
returned action chunk keeps the same layout. See docs/推理部署方案.md.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TienKung dual-arm HTTP inference server.")
    parser.add_argument("--host", default="0.0.0.0", help="Bind host.")
    parser.add_argument("--port", type=int, default=18000, help="Bind port.")
    parser.add_argument("--config-name", required=True, help="TrainConfig name, e.g. pi05_tienkung_lint_roller_delta.")
    parser.add_argument(
        "--checkpoint-dir",
        required=True,
        type=pathlib.Path,
        help="Checkpoint step directory, e.g. checkpoints/<config>/<exp>/29999.",
    )
    parser.add_argument(
        "--assets-dir",
        type=pathlib.Path,
